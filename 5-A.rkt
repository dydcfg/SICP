...
(define (install-raise-package) ;install raise functions 
  ...
  (define (raise-rational n)
    (make-real (/ (car (contents n)) (* 1.0 (cdr (contents n))))))
  (define (raise-real n)
...
(define (raise-time a time)
  (if (= time 0)
      a
      (raise-time ((get 'raise (type-tag a)) a) (- time 1))))

(define (raise-to-complex a)
  (if (eq? (type-tag a) 'complex)
      a
      (raise-to-complex (raise-time a 1))))



(define (diff-rank type-tags)
  (define (get-rank x)
    (cond ((eq? x 'integer)
           0)
          ((eq? x 'rational)
          ...
          ((eq? x 'complex)
           3)))
  (- (get-rank (cadr type-tags)) (get-rank (car type-tags))))
    ...
    (let ((proc (get op type-tags)))
      (if proc
          ...
          (if (= (length args) 2)
              (let ((a1 (car args))
                    (a2 (cadr args)))
                (let ((diff (diff-rank type-tags)))
                  (cond ((> diff 0)
                         (apply-generic op (raise-time a1 diff) a2))
                        (else 
                         (apply-generic op a1 (raise-time a2 (- 0 diff)))))))
              (let ((a1 (car args)))
                (apply-generic op (raise-to-complex a1))))))))

(define (display-obj x)
  (cond ((eq? (type-tag x) 'rational)
         (displayln (list (type-tag x) (car (contents x)) (cdr (contents x)))))
        ((eq? (type-tag x) 'complex)
         (displayln (list (type-tag x) (car (contents (contents x))) (cdr (contents (contents x))))))
        (else
         (displayln (list (type-tag x) (contents x))))))
